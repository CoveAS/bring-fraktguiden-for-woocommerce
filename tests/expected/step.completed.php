<!-- Test with href -->
<div class="bfg-step bfg-step--completed">
	<div class="bfg-step__icon bfg-step__icon--completed">
		<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
			<polyline points="20 6 9 17 4 12"></polyline>
		</svg>
	</div>
	<div class="bfg-step__content">
		<a href="/settings" class="bfg-step__link"><?php esc_html_e('Setup API credentials', 'bring-fraktguiden-for-woocommerce'); ?></a>
		
	<p class="bfg-step__desc">Configure your Bring API keys</p>
	<span class="bfg-badge bfg-badge--completed"><?php esc_html_e('Completed', 'bring-fraktguiden-for-woocommerce'); ?></span>

	</div>
</div>

<!-- Test without href -->
<div class="bfg-step bfg-step--completed">
	<div class="bfg-step__icon bfg-step__icon--completed">
		<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
			<polyline points="20 6 9 17 4 12"></polyline>
		</svg>
	</div>
	<div class="bfg-step__content">
		<a href class="bfg-step__link"><?php esc_html_e('Verify installation', 'bring-fraktguiden-for-woocommerce'); ?></a>
		
	<p class="bfg-step__desc">Check plugin is active</p>
	<span class="bfg-badge bfg-badge--completed"><?php esc_html_e('Completed', 'bring-fraktguiden-for-woocommerce'); ?></span>

	</div>
</div>

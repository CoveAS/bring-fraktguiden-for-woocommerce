...
class Bring_Booking_Common_View {
  ...
  static function render_shipping_date_time( $name = '_bring-shipping-date' ) {
    ...
    echo '<input type="text" name="' . $name . '-hour" value="' . $shipping_date['hour'] . '" maxlength="2" placeholder="' . __( 'hh', self::TEXT_DOMAIN ) . '" style="width:3em;text-align:center">:';
    echo '<input type="text" name="' . $name . '-minutes" value="' . $shipping_date['minute'] . '" maxlength="2" placeholder="' . __( 'mm', self::TEXT_DOMAIN ) . '" style="width:3em;text-align:center">';
  }
  ...
  static function get_booking_status_info( $order ) {
    $result = [
        'text' => __( 'No', self::TEXT_DOMAIN ),
        'icon' => 'dashicons-minus',
    ];

    if ( self::is_step2() ) {
      $result = [
          'text' => __( 'In progress', self::TEXT_DOMAIN ),
          'icon' => '',
      ];
    }
    ...
    if ( $order->is_booked() ) {
      $result = [
          'text' => __( 'Booked', self::TEXT_DOMAIN ),
          'icon' => 'dashicons-yes',
      ];
    }
    ...
    if ( $order->has_booking_errors() ) {
      $result = [
          'text' => __( 'Failed', self::TEXT_DOMAIN ),

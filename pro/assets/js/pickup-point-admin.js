...
(function () {
    ...
    Shipping_Item.prototype = {
        ...
        on_update_rate: function () {
            ...
            if ( service ) {
                ...
                Data.get_rate( service, function ( response, status ) {
    ...
    function add_order_item_event_handlers() {
        ...
        $( document.body ).on( 'order_items_loaded.bfg', function () {
            Data.load_bring_shipping_info();
        } );


        $( document.body ).on( 'bring_info_loaded.bring', function () {
            Shipping_Items_Manager.clear();

            shipping_order_items().each( function ( i, elem ) {
        ...
        order_items.on( 'click', '#order_shipping_line_items a.edit-order-item', function () {
        ...
        order_items.on( 'change', 'select[name^=shipping_method]', function () {
            var shipping_item = Shipping_Items_Manager.from_row( $( this ).closest( 'tr' ) );
        ...
        order_items.on( 'change', 'select[name^=_fraktguiden_services]', function () {
            var shipping_item = Shipping_Items_Manager.from_row( $( this ).closest( 'tr' ) );
        ...
        order_items.on( 'keyup', '[name^=_fraktguiden_pickup_point_postcode]', function () {
            var shipping_item = Shipping_Items_Manager.from_row( $( this ).closest( 'tr' ) );
        ...
        order_items.on( 'change', 'select[name^=_fraktguiden_pickup_point_selector]', function () {
            var shipping_item = Shipping_Items_Manager.from_row( $( this ).closest( 'tr' ) );
            shipping_item.on_pickup_point_changed();
        } );


        order_items.on( 'click', '.get-rate-button', function ( evt ) {
            evt.preventDefault();
            var shipping_item = Shipping_Items_Manager.from_row( $( this ).closest( 'tr' ) );

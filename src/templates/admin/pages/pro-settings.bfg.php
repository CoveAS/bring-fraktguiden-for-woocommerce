...
function init_form_fields_for_mybring() {
	...
	$form_fields['booking_return_service'] = [
		...
		'desc_tip' => sprintf(
			...
			__( 'Bring only adds a return label to a shipment inside Norway, on these services: %s. Any other service books without one.', 'bring-fraktguiden-for-woocommerce' ),
			implode( ', ', \BringFraktguiden\Booking\ReturnLabel::product_names() )
		),
		'options'  => [ 'none' => __( 'No return label', 'bring-fraktguiden-for-woocommerce' ) ] + \BringFraktguiden\Booking\ReturnLabel::titles(),
		'default'  => 'none',
	];
